...
function AddProveedorModal({ isOpen, onClose, onSuccess, centroId, sedeId, proveedores, aplicaciones, categorias = [], itemToEdit }) {
  ...
  const handleInputChange = (e) => {
    ...
    if (name === 'proveedor_id') newFormData.aplicacion_id = '';
    if (name === 'categoria') newFormData.detalles = {};
  ...
  const handleSubmit = async (e) => {
    ...
    toast.promise(actionPromise, {
      loading: 'Guardando...',
      success: () => { onSuccess(); return `¡Guardado con éxito!`; },
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black bg-opacity-60 p-4">
      <div className="w-full max-w-2xl rounded-xl bg-white p-6 shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="mt-4 max-h-[70vh] overflow-y-auto pr-2 space-y-4">
          <div>
            <label className={labelStyle}>Categoría del Servicio</label>
          ...
          {formData.categoria && (
            <>
              <div>
                ...
                <select name="proveedor_id" value={formData.proveedor_id || ''} onChange={handleInputChange} className={inputStyle} required>
              ...
              {categoriaSeleccionada?.campos_formulario.map(field => (
                <div key={field.name}>
