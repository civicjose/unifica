      ...
      <table className="min-w-full">
        ...
        <tbody className="bg-white">
          {trabajadores.map((trabajador, index) => {
            const isLastRow = index >= trabajadores.length - 3;
            const menuPositionClass = isLastRow ? 'bottom-full mb-2' : 'top-full mt-2';
                ...
                <td onClick={(e) => e.stopPropagation()} className="whitespace-nowrap px-6 py-4 text-right">
                  <div className="relative inline-block text-left" ref={openMenuId === trabajador.id ? menuRef : null}>
                    ...
                    {openMenuId === trabajador.id && (
                      ...
                      <div className={`absolute right-0 z-10 w-48 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 ${menuPositionClass}`}>
                        <div className="py-1">
