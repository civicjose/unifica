...
function CentrosPage() {
  ...
  const handleDeleteClick = (centro) => { setItemToDelete(centro); };
  ...
  return (
    <>
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <h1 className="text-3xl font-bold text-gray-800">Centros y Servicios</h1>
      ...
      {loading ? ( <p className="p-4 text-center">Cargando centros...</p> ) : (
        <div className="space-y-8">
          {Object.keys(groupedCentros).length > 0 ? (
            Object.keys(groupedCentros).sort().map((tipo) => (
                <div key={tipo}>
                  <h2 className="text-2xl font-bold text-gray-700 mb-4 border-b-2 border-primary/50 pb-2">{tipo}</h2>
                  <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                    {groupedCentros[tipo].map((centro) => (
                      <CentroCard key={centro.id} centro={centro} onEdit={handleOpenEditModal} onDelete={handleDeleteClick}/>
                    ))}
                  </div>
                </div>
              ))
          ) : (
            <p className="text-center text-slate-500 py-8">No se encontraron centros con los filtros actuales.</p>
          )}
        </div>
      )}

      <CentroModal isOpen={isModalOpen} onClose={() => { setIsModalOpen(false); setItemToEdit(null); }} onSuccess={handleModalSuccess} itemToEdit={itemToEdit} listas={listas}/>
      <ConfirmModal isOpen={itemToDelete !== null} onClose={() => setItemToDelete(null)} onConfirm={handleConfirmDelete} title="Eliminar Centro">
        <p>¿Seguro que quieres eliminar <strong>{itemToDelete?.nombre_centro}</strong>? Esta acción no se puede deshacer.</p>
      </ConfirmModal>
    </>
  );
}
